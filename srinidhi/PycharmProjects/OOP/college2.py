...
class Address(object):
    ...
    def __init__(self, h_no, street_name, city_name, state, pin_code):
        ...
        print self.h_no, self.street_name, self.city_name, self.state, self.pin_code
...
class Person(object):
    ...
    def change_address(self,h_no,street_name,city_name,state,pin_code):
        self.address = Address(h_no, street_name, city_name, state, pin_code)
...
class College(object):
    ...
    def subscribe_course(self, roll_no, course):
        list_course=course.split(',')
        self.list_of_student_course.update({roll_no:list_course})
        print self.list_of_student_course

    def unsubscribe_course(self, roll_no, course):
        flag = 0
        for i in range(0, len(self.list_of_student_course[roll_no])):
            if course == self.list_of_student_course[roll_no][i]:
                flag = 1
    ...
    def change_of_address(self):
        choice = raw_input("enter s or t :")

        if choice == 's':
            roll_no = int(raw_input("roll No "))
            flag = 0
            for i in range(0, len(self.list_of_student)):
                if roll_no == self.list_of_student[i].roll_no:
                    flag = 1
                    self.list_of_student[i].address = Person.change_address(self, raw_input("h no "), raw_input("street name "), raw_input("city name "), raw_input("state name "), raw_input("pin "))
                    break
            if flag == 0:
                print "Roll No %d Not Present" % roll_no
...
if __name__ == '__main__':
    ...
    address_default = Address("1-2/3", "Street", "City", "TS", "500000")
    c = College("aaa", address_default, "9999999999")


    while(1):

        print "1. add_teacher  2. remove_teacher  3. add_student  4. remove_student  5. subscribe_course  6. unsubscribe_course  7. search_student  8. list_student  9. list_course  10. exit  11. display"
        ...
        if choice == 1:
            ...
            s = Student(Person(raw_input("name"),raw_input("dob"), Address(raw_input("h no "), raw_input("street name ") , raw_input("city name "), raw_input("state name ") , raw_input("pin ")),
